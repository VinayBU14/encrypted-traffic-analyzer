...
def render() -> None:
    ...
    if st.session_state.get("live_capture_running", False):
        interface = st.session_state.get("live_interface", "Wi-Fi")

        try:
            reader = LiveCaptureReader(interface=interface, bpf_filter="")

            packets_this_cycle = 0
            for raw_pkt in reader.start_capture():
                if not st.session_state.get("live_capture_running", False):
                    break
                ...
                if packets_this_cycle >= 10:
                    break
        except Exception as exc:
            _capture_active = False
            st.session_state["live_capture_running"] = False
            st.error(f"Capture error: {exc}")
            return
